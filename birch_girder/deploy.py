...
def get_paginated_results(product, action, key, credentials=None, args=None):
    ...
    return [
        ...
        for sublist in [
            x[key]
            for x in boto3.client(product, **credentials)
...
def main():
    ...
    parser.add_argument(
        ...
        default="birch-girder",
        help="Name of the IAM role to be used by Lambda (default: %(default)s)",
    )
    parser.add_argument(
        "--ses-rule-set-name",
    ...
    parser.add_argument(
        ...
        help="Name of the IAM user to be used by GitHub (default: %(default)s)",
    )
    parser.add_argument(
        "--lambda-archive-filename",
        metavar="FILENAME.ZIP",
        help="Path to the newly generated lambda zip file (default: temporary file)",
    )
    parser.add_argument(
        "--github-action-filename",
        default="emit-comment-to-sns-github-action.yml",
        metavar="FILENAME.yml",
        help="Filename to use for the GitHub Actions workflow (default: %(default)s)",
    )
    parser.add_argument(
        "--plugins-path",
        default="plugins",
        type=plugin_path_type,
...
Error "{repr(e)}"''')
    ...
    early_run = False
    if args.lambda_function_name not in [
        x["FunctionName"] for x in response["Functions"]
    ]:
...
Girder will use to interact with GitHub""")
        ...
        scopes = [
            "repo",
            (
                # This is required for modification of the GitHub Actions file
        ...
        # We can't move from classic to fine grained PATs yet because it's not implemented
        # https://github.com/github/roadmap/issues/601
        # When it's time for fine grained PATS the scopes will need to include
        # None
        # * gh.search.issues.get
        #
        # "Contents" repository permissions (write)
        # * gh.repos[owner][repo].contents[path].put
        # * gh.repos[owner][repo].contents[path].get
        # * gh.markdown.post
        #
        # "Issues" repository permissions (write)
        # * gh.repos[owner][repo].issues[issue_data['number']].reactions.post
        # * gh.repos[owner][repo].issues.post
        # * gh.repos[owner][repo].issues[issue_number].get
        # * gh.repos[owner][repo].issues[issue_number].patch
        # * gh.repos[owner][repo].issues[issue_number].comments.post
        # * gh.repos[owner][repo].issues[issue_number].comments.get
        # * gh.repos[owner][repo].issues[issue_number].comments.patch
        # * gh.repos[owner][repo].issues[issue_number].comments[comment_id].reactions.post
        #
        ...
        # * deploy.py :
        #   gh.repos[owner][repo].contents['.github']['workflows'][github_action_filename].put

        auth = GitHub(config["github_username"], password)
        status, authorization_data = auth.authorizations.post(
            body={"scopes": scopes, "note": note, "note_url": note_url}
        )
        if status == 401 and "required" in dict(auth.getheaders()).get("x-github-otp"):
        ...
        green_print(
            f"GitHub OAuth Token (github_token) created : {config['github_token']}"
        )
...
an SNS topic created that internal Birch Girder errors will be sent to.""")
    ...
    try:
        ...
        response = client.create_bucket(
            ...
            CreateBucketConfiguration={"LocationConstraint": config["sns_region"]},
        )
        green_print(f"AWS S3 Bucket {response['Location']} created")

    statement_id = "GiveSESPermissionToWriteEmail"
    try:
        response = client.get_bucket_policy(Bucket=config["ses_payload_s3_bucket_name"])
    ...
    if statement_id not in [x["Sid"] for x in policy["Statement"]]:
        policy["Statement"].append({
            ...
            "Condition": {
                "StringEquals": {"aws:Referer": config["sns_topic_arn"].split(":")[4]}
            },
        })
        ...
        green_print(
            f"AWS S3 Bucket policy for {config['ses_payload_s3_bucket_name']} created"
        )
    ...
    if rules is None or lifecycle_id not in [x["ID"] for x in rules]:
        client.put_bucket_lifecycle_configuration(
            ...
            LifecycleConfiguration={
                "Rules": [{
                    ...
                    "ID": lifecycle_id,
                    "Filter": {"Prefix": config["ses_payload_s3_prefix"]},
                    "Status": "Enabled",
        ...
        green_print(
            "AWS S3 Bucket lifecycle configuration for "
    ...
    for recipient in [x.lower() for x in list(config["recipient_list"].keys())]:
        ...
        if recipient not in identities and domain not in identities:
            ...
            if response.lower() in ["email", recipient]:
                client.verify_email_identity(EmailAddress=recipient)
                ...
                response = prompt(
                    ...
                    "(for $0.50/month) or on your own [route53/myself]"
                )
                if response.lower() in ["route53", "r"]:
                    print("Route53 support in Birch Girder is not yet available.")
                elif response.lower() in ["myself", "m"]:
                    zone = ".".join(domain.split(".")[-2:])
                    record = ".".join(domain.split(".")[:-2]) if zone != domain else "@"
                    ...
                    dkim_cname_records = "\n".join([
                        f"{x}._domainkey{suffix}    IN    CNAME   "
                        f" {x}.dkim.amazonses.com."
                    ...
                    print(
                        f"""
To verify this domain create a DNS record in the {domain} domain with
...
"""
            ...
            identities_that_matter.append(domain if domain in identities else recipient)
    if verifications_initiated:
        print(
            """Aborting while you complete email/domain verifications. Run this again when
they're complete"""
        )
        return
    ...
    for identity in response["VerificationAttributes"]:
        ...
        if status == "Pending":
            print(
                f"Verification for {identity} is still pending. Run this "
            ...
            print(
                f"Verification for {identity} has been deleted. "
                "Run this again to initiate a new verification."
            )
            all_verifications_completed = False
        elif status == "TemporaryFailure":
            print(
                f"Verification for {identity} has temporarily failed. Wait "
                "and run this again."
            )
            all_verifications_completed = False
    ...
    recipient_email_address_list = [
        x.lower() for x in list(config["recipient_list"].keys())
    ]
    client = boto3.client("ses")
    ...
    for recipient_email_address in recipient_email_address_list:
        if recipient_email_address in response["Identities"]:
            recipient_to_ses_identity_map[recipient_email_address] = response[
                "Identities"
            ][recipient_email_address]
        elif recipient_email_address.partition("@")[2] in response["Identities"]:
            recipient_to_ses_identity_map[recipient_email_address] = (
                recipient_email_address.partition("@")[2]
            )
        else:
    ...
    # TODO : What if we exceed the 1 call/sec limit to the endpoint that sets these values?
    for recipient, identity in recipient_to_ses_identity_map.items():
        if identity in processed_identities:
            # We've already processed this identity due to another recipient
            # in the same domain, skipping
            continue
        ...
        for attribute in ["Bounce", "Complaint", "Delivery"]:
            if identity is None:
                ...
                green_print(
                    f"New SES Notification for {attribute} notifications for"
                    f" {recipient} set to {config['ses_notification_sns_topic_arn']}"
                )
            elif (
                f"{attribute}Topic" not in existing_attributes[identity]
                or existing_attributes[identity][f"{attribute}Topic"]
                != config["ses_notification_sns_topic_arn"]
            ):
                ...
                green_print(
                    f"Existing SES Notification for {attribute} notifications for"
                    f" {identity} changed from"
                    f" {existing_attributes[identity].get(f'{attribute}Topic', None)} to"
                    f" {config['ses_notification_sns_topic_arn']}"
                )
  ...
  "Statement": [
    ...
    if args.lambda_iam_role_name in [x["RoleName"] for x in iam_roles]:
        lambda_iam_role_arn = next(
            x["Arn"] for x in iam_roles if x["RoleName"] == args.lambda_iam_role_name
        )
    else:
    ...
    with open(zip_file_name, mode="rb") as f:
        ...
        if layer_name not in [x["LayerName"] for x in layers]:
            ...
            if digest not in hash_map:
                ...
                try:
                    ...
                    if birch_girder_layer_version_arn not in [
                        x["Arn"] for x in response["Configuration"].get("Layers", [])
                    ]:
                        # Lambda function doesn't include the layer
                        function_needs_update = (
                            "there is a new layer being added to the function"
                        )
                except Exception:
        ...
        if publish_layer:
            ...
            green_print(
                f"AWS Lambda layer published : {birch_girder_layer_version_arn}"
            )
    ...
    if args.lambda_function_name not in [x["FunctionName"] for x in functions]:
        ...
        while True:
            try:
                ...
                if "The role defined for the function cannot be assumed" in str(e):
        ...
        if function_needs_update:
            ...
            green_print(
                "AWS Lambda function configuration updated with new Lambda layer"
                f" because {function_needs_update} :{response['FunctionArn']}"
            )
        pass
    ...
    with zipfile.ZipFile(in_memory_data, "w") as zip_file:
        ...
        for filename in [
            x for x in os.listdir("birch_girder") if x not in excluded_files
        ]:
            arcname = os.path.join("birch_girder", filename)
            zip_file.write(arcname, arcname, zipfile.ZIP_DEFLATED)
        ...
        for filename in [x for x in os.listdir(args.plugins_path) if x.endswith(".py")]:
            ...
            zip_file.write(full_path, arcname, zipfile.ZIP_DEFLATED)
    ...
    while True:
        try:
            ...
            # An update is in progress, so we wait
            time.sleep(2)
    lambda_function_arn = response["FunctionArn"]
    ...
    if response["MaximumRetryAttempts"] != 0:
        response = client.update_function_event_invoke_config(
            FunctionName=args.lambda_function_name, MaximumRetryAttempts=0
        )
        green_print(
    ...
    try:
        ...
        policy = None
    if policy is None or statement_id not in [
        ...
        green_print(
            f"Permission {statement_id} added to AWS Lambda function "
    ...
    while True:
        try:
            ...
            if not rule_set_created:
                client.create_receipt_rule_set(RuleSetName=args.ses_rule_set_name)
    ...
    if args.ses_rule_name not in [x["Name"] for x in response["Rules"]]:
        ...
        green_print(
            f"AWS SES Rule {args.ses_rule_name} created in Rule Set "
            f"{args.ses_rule_set_name}"
        )
    response = client.describe_active_receipt_rule_set()
    if response["Metadata"]["Name"] != args.ses_rule_set_name:
        client.set_active_receipt_rule_set(RuleSetName=args.ses_rule_set_name)
    ...
    if args.github_iam_username not in [x["UserName"] for x in iam_users]:
        response = client.create_user(UserName=args.github_iam_username)
    ...
    if policy_name not in user_policies:
        ...
        green_print(
            f"AWS IAM policy {policy_name} applied to user {args.github_iam_username}"
        )
    ...
    for owner, repo, repo_private in set([
        (
            ...
            config["recipient_list"][x].get("repo_private", True),
        )
        for x in config["recipient_list"]
    ]):
        ...
        if repo_data.get("name") is None:
            body = {"name": repo, "private": repo_private, "auto_init": True}
            if owner != user_data["login"]:
                ...
                if org_data.get("login") is None:
                    print(
                        f"""
  Recipient {html_url} has repo owner of {owner} but the github_token user we're
...
organization so we can't create the repo. Skipping"""
                    ...
                    status, repo_data = org.repos.post(body=body)
            else:
                status, repo_data = gh.user.repos.post(body=body)
            if status == 422:
                print(
                    f"  Got error {status} when attempting to create new "
                    f"GitHub repo {repo}"
                )
                return
        ...
        if "BIRCH_GIRDER_AWS_ACCESS_KEY_ID" not in [
            x["name"] for x in secrets_data["secrets"]
        ]:
            if not config.get("github_iam_user_access_key_id"):
                response = client.create_access_key(UserName=args.github_iam_username)
                config["github_iam_user_access_key_id"] = response["AccessKey"][
                    "AccessKeyId"
                ]
                config["github_iam_user_secret_access_key"] = response["AccessKey"][
                    "SecretAccessKey"
                ]
                ...
                green_print(
                    "Created new Access Key for AWS IAM user "
                    f"{args.github_iam_username} : "
                    f"{config['github_iam_user_access_key_id']}"
                )

            status, public_key_data = (
                gh.repos[owner][repo].actions.secrets["public-key"].get()
            )

            status, _ = gh.repos[owner][
                repo
            ].actions.secrets.BIRCH_GIRDER_AWS_ACCESS_KEY_ID.put(
                body={
                    "encrypted_value": encrypt_github_actions_secret(
                        public_key_data["key"], config["github_iam_user_access_key_id"]
                    ),
            ...
            status, _ = gh.repos[owner][
                repo
            ].actions.secrets.BIRCH_GIRDER_AWS_SECRET_ACCESS_KEY.put(
                body={
                    "encrypted_value": encrypt_github_actions_secret(
                        public_key_data["key"],
        ...
        status, workflow_data = (
            gh.repos[owner][repo]
            .contents[".github"]["workflows"][args.github_action_filename]
            .get()
        )
        with open("emit-comment-to-sns-github-action.yml") as f:
            workflow_config = yaml.load(f.read(), Loader=yaml.SafeLoader)
            workflow_config["jobs"]["emit_comment"][
                "if"
            ] = f"github.event.comment.user.login != '{config['github_username']}'"
            workflow_config["jobs"]["emit_comment"]["env"][
                "BIRCH_GIRDER_SNS_TOPIC_REGION"
            ] = config["sns_topic_arn"].split(":")[3]
            workflow_config["jobs"]["emit_comment"]["env"][
                "BIRCH_GIRDER_SNS_TOPIC_ARN"
            ...
            if status == 404:
                status, _ = (
                    ...
                    .put(
                        body={
                            "message": (
                                "Adding Birch Girder GitHub Actions"
                ...
                status, _ = (
                    ...
                    .put(
                        body={
                            "message": (
                                "Updating Birch Girder GitHub Actions"
                                " workflow\n\nhttps://github.com/gene1wood/birch-girder"
                            ),
                            "content": b64_content,
                ...
                if 200 <= status < 300:
                    ...
                    print(
                        "result from update"
                        f" {owner}/{repo}/.github/workflows/{args.github_action_filename}"
                        f" {status} and {_}"
                    )
    ...
    if lambda_function_arn not in [
        x["Endpoint"] for x in subscriptions if x["Protocol"] == "lambda"
    ]:
        ...
        green_print(
            "Birch Girder AWS Lambda function subscribed to AWS SNS "
    ...
    if lambda_function_arn not in [
        ...
        green_print(
            "Birch Girder AWS Lambda function subscribed to AWS SES "
