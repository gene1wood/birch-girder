...
def fetch_replay(s3_payload_filename, ses_payload_s3_bucket_name, ses_payload_s3_prefix):
...
def process_event(event, context, config):
    ...
    try:
        ...
        if event_type == 'ses_email':
            ...
            process_github_webhook.process_github_webhook(event, persistent_data, config)
        elif event_type == 'ses_notification':
            process_ses_notification.process_ses_notification(event, persistent_data, config['github_token'])
            ...
            process_email.process_email(synthetic_event, persistent_data, config, send_email)
