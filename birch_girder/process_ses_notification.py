...
def process_ses_notification(event, persistent_data, github_token):
    ...
    if ses_notification["notificationType"] == "Bounce":
        ...
        raise Exception(
            "Unexpected AWS SES notificationType of"
            f" #{ses_notification['notificationType']} in event"
        )

    message_id = ses_notification["mail"]["messageId"]
    # Note : We don't want self.event['Records'][0]['Sns']['MessageId'] as it's the
    # outer message ID which we're not using
    if message_id in persistent_data["sent_mail"]:
        ...
        if "comment_id" in issue_or_comment:
            ...
            reaction_target = gh.repos[issue_or_comment["repo_owner"]][
                issue_or_comment["repo_name"]
            ].issues.comments[issue_or_comment["comment_id"]]
            logger.info(
                f"Adding reaction {reaction_content} to comment"
                f" {issue_or_comment['comment_id']} in repo"
                f" {issue_or_comment['repo_owner']}/{issue_or_comment['repo_name']} due"
                " to SES notification"
            )

        elif "issue_number" in issue_or_comment:
            # The email was sent in response to the creation of an issue
            repo = gh.repos[issue_or_comment["repo_owner"]][
                issue_or_comment["repo_name"]
            ]
            reaction_target = repo.issues[issue_or_comment["issue_number"]]
            logger.info(
                f"Adding reaction {reaction_content} to issue"
                f" {issue_or_comment['issue_number']} in repo"
                f" {issue_or_comment['repo_owner']}/{issue_or_comment['repo_name']} due"
                " to SES notification"
            )
        else:
            raise Exception(
                "The information stored in the PersistentMap['sent_mail'] for message "
        ...
        raise Exception(
            f"An SES notification was received with message_id {message_id} which"
            " doesn't map to an email that we've seen before. persistent_data is"
            f" {persistent_data} and the event is {event}"
        )
    ...
    if ses_notification["notificationType"] in ["Bounce", "Complaint"]:
        ...
        if "comment_id" in issue_or_comment:
            comment = gh.repos[issue_or_comment["repo_owner"]][
                issue_or_comment["repo_name"]
            ].issues.comments[issue_or_comment["comment_id"]]
            status, comment_update_result = (
                ...
                .issues.comments[issue_or_comment["comment_id"]]
                .patch(
                    body={"body": comment["body"] + reaction_details},
                    headers={"Accept": "application/vnd.github+json"},
                )
            )
        elif "issue_number" in issue_or_comment:
            status, issue = gh.repos[issue_or_comment["repo_owner"]][
                issue_or_comment["repo_name"]
            ].issues.issue_or_comment["issue_number"]
            status, issue_update_result = (
                gh.repos[issue_or_comment["repo_owner"]][issue_or_comment["repo_name"]]
                .issues.issue_or_comment["issue_number"]
                .patch(
                    body={"body": issue["body"] + reaction_details},
                    headers={"Accept": "application/vnd.github+json"},
                )
            )
        else:
