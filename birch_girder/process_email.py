...
from .utils import (
    parse_hidden_content,
    update_issue,
    get_content_block,
    produce_attachment_table,
    send_email,
)
...
class Email:
    def __init__(self, config, event, dryrun=False):
        ...
        self.raw_subject = self.record["ses"]["mail"]["commonHeaders"]["subject"]
        self.from_address = ""
        self.source = ""
        self.replyto = ""
        self.to_address = ""
        self.s3_payload_filename = self.record["ses"]["mail"]["messageId"]
        self.message_id = self.record["ses"]["mail"]["commonHeaders"].get("messageId")
    ...
    def parse_email(self):
        ...
        self.from_address = (
            ...
            else ", ".join(self.record["ses"]["mail"]["commonHeaders"]["from"])
        )
        ...
        for possible_recipient in possible_recipients:
            ...
            if possible_recipient.lower() in [
                x.lower() for x in self.record["ses"]["mail"]["destination"]
            ]:
        ...
        if not self.to_address:
            ...
            logger.debug(
                "No applicable email was found in destination list "
                f"so we will use {self.to_address} : "
                f"{self.record['ses']['mail']['destination']}"
            )

            logger.critical(
                "None of the 'To' addresses in"
                f" '{self.record['ses']['mail']['destination']}' were found in"
                f" the recipient_list '{possible_recipients}'. As a result we're just"
                f" going to assign {self.to_address} as the 'To' address. This means"
                " that the ticket may have the wrong metadata in the GitHub issue"
                " hidden content and the reply email back to the submitter may come"
                " from a different email address than they sent the request to."
            )

        if "replyTo" in self.record["ses"]["mail"]["commonHeaders"]:
            self.replyto = self.record["ses"]["mail"]["commonHeaders"]["replyTo"][0]
        try:
            self.source = clean_sender_address(self.record["ses"]["mail"]["source"])
        except Exception as e:
            logger.error(
                "Failed to clean sender address "
                f"{self.record['ses']['mail']['source']} due to \"{e}\""
            )

        self.github_owner = self.config["recipient_list"][self.to_address].get("owner")
        self.github_repo = self.config["recipient_list"][self.to_address].get("repo")
        ...
        body_to_parse = (
            self.email_body_text if self.email_body_text != "" else self.email_body
        )
    ...
    def parse_subject(self):
        ...
        :return:
        """
        stripped_subject = EMAIL_SUBJECT_PREFIX.sub("", self.raw_subject).strip()
        ...
        for issue_search_result in data["items"]:
            ...
            issue_source = (
                issue_data["source"]
                if "source" in issue_data
                else issue_data.get("from")
            )
            if self.source in issue_source:
                results_list.append(issue_search_result["number"])
            else:
                logger.debug(
                    "Encountered an inbound email that has a subject which matches"
                    f" issue #{issue_search_result['number']} in"
                    f" {self.github_owner}/{self.github_repo} but which has a mail"
                    f" envelope MAILFROM source of {self.source} not {issue_source} who"
                    " created the existing issue. Creating a new issue."
                )
    ...
    def get_email_payload(self):
        ...
        logger.debug(
            f"Using waiter to wait for {bucket} {key} to persist through s3 service"
        )
    ...
    def parse_email_payload(self):
        ...
        for part in msg.walk():
            ...
            # HTML body
                ...
                self.email_body = "".join(
                    str(x)
                    for x in (
                        soup.body.contents if soup.body is not None else soup.contents
                    )
        ...
        for part in msg.iter_attachments():
            ...
            status, data = (
                self.gh.repos[self.github_owner][self.github_repo]
                .contents[path]
                .put(
                    body={
            ...
            if int(status / 100) != 2:
                if status == 422 and '"sha" wasn\'t supplied' in data.get(
                    "message", ""
                ):
                    status, data = (
                        self.gh.repos[self.github_owner][self.github_repo]
                    ...
                    logger.info("Attachment already exists; referencing existing file")
...
def send_email_to_reporter(parsed_email, issue_data, persistent_data, config, dryrun):
    ...
    if "known_machine_senders" in config:
        known_machine_senders = [x.lower() for x in config["known_machine_senders"]]
        ...
        if parsed_email.from_address.lower() in known_machine_senders:
            ...
            should_send_email = False
        if parsed_email.replyto != "" and parsed_email.replyto in known_machine_senders:
            should_send_email = False
        if not should_send_email:
            logger.info(
                f"Not sending an email to {parsed_email.source}/"
                f"{parsed_email.from_address}/{parsed_email.replyto} "
                "because they are a known machine sender."
            )
            return None
    body = (
...
to add to your request."""
    ...
    status, html_body = gh.markdown.post(
        body={
            "text": body,
            "mode": "gfm",
            "context": "/".join([parsed_email.github_owner, parsed_email.github_repo]),
        }
    )
    text_url = (f"https://github.com/{parsed_email.github_owner}"
                f"/{parsed_email.github_repo}/issues/{issue_data['number']}")
    issue_reference = (
        f"{parsed_email.github_owner}/{parsed_email.github_repo}#{issue_data['number']}"
    )
    html_url = f'<a href="{text_url}">{issue_reference}</a>'
    email_subject = SUBJECT_TEMPLATE.substitute(
        subject=parsed_email.subject, issue_number=issue_data["number"]
    )
    ...
    logger.info(
        ...
        "that a new issue has been created."
    )
    if dryrun:
        return "1"
    template_args = {
    ...
    message_id = send_email(
        ...
        from_name=(config["recipient_list"][parsed_email.to_address].get("name")),
        ...
        html=EMAIL_HTML_TEMPLATE.substitute(
            html_body=html_body.decode("utf-8").format(html_url), **template_args
        ),
        text=EMAIL_TEXT_TEMPLATE.substitute(
            text_body=body.format(text_url), **template_args
        ),
    )
    ...
    logger.debug(
        f"Stored sent email to reporter in persistent data for message_id {message_id}"
    )
...
def create_issue(repo, parsed_email, config, dryrun=False):
    ...
    issue_message = ISSUE_TEMPLATE.substitute(
        hidden_content_block=get_content_block(
            "hidden_content", yaml.safe_dump(email_metadata, default_flow_style=False)
        ),
        ...
        headers=json.dumps(parsed_email.record["ses"]["mail"]["headers"]),
        body=parsed_email.stripped_reply,
        attachment_table=get_content_block(
            "attachments", produce_attachment_table(parsed_email.new_attachment_urls)
        ),
    )
    if not dryrun:
        ...
        issue_data = {"number": 1}
    logger.info(f"Created new issue {issue_data['number']}.")
    return issue_data
...
def clean_sender_address(sender):
    ...
    https://github.com/rspamd/rspamd/blob/master/src/plugins/lua/aliases.lua
    ...
    # prvs=billing@example.com=4480132787
    # tag_value = 4480132787
    # loc_core = billing@example.com
    ...
    if len(elements) == 3:
        try:
            int(elements[1], 16)
            # tag_val is the second element and loc_core is the third
            loc_core = elements[2]
        except ValueError:
            try:
                int(elements[2], 16)
                # tag_val is the third element and loc_core is the second
                loc_core = elements[1]
            except ValueError:
                raise Exception(
...
def add_comment_to_issue(issue, parsed_email, dryrun=False):
    ...
    comment_message = COMMENT_TEMPLATE.substitute(
        ...
        to_address=parsed_email.to_address,
        date=parsed_email.date,
        headers=json.dumps(parsed_email.record["ses"]["mail"]["headers"], indent=4),
        body=parsed_email.stripped_reply,
        comment_attachments=comment_attachments,
    )
    logger.info(f"Adding a comment to the existing issue {parsed_email.issue_number}.")
    if not dryrun:
        status, comment_data = issue.comments.post(body={"body": comment_message})
...
def process_email(event, persistent_data, config, send_email=True):
    ...
    if len(event["Records"]) > 1:
        raise Exception(f"Multiple records from SES {event['Records']}")

    dryrun_tag = (
        config["dryrun_tag"] if "dryrun_tag" in config else "--#_##DRYRUN##_#--"
    )
    if dryrun_tag in event["Records"][0]["ses"]["mail"]["commonHeaders"]["subject"]:
        dryrun = True
    ...
    plugin_list = [
        ...
        if hasattr(x, "is_matching_email") and hasattr(x, "transform_email")
    ]

    parsed_email = Email(config, event, dryrun)
    ...
    for plugin in plugin_list:
        ...
        if is_matching_email(parsed_email):
            ...
            logger.debug(f"Incoming email did not match plugin {plugin.__name__}")
    ...
    repo = gh.repos[parsed_email.github_owner][parsed_email.github_repo]
    if parsed_email.issue_number:
        ...
        if not send_email:
            ...
            return
        message_id = send_email_to_reporter(
            parsed_email, issue_data, persistent_data, config, dryrun
        )
        if message_id is not None:
            logger.debug(f"Initial email reply sent with Message-ID {message_id}")
