...
def verify_email(username):
    ...
    identities = get_paginated_results(
        "ses", "list_identities", "Identities", {"IdentityType": "EmailAddress"}
    )
    ...
    if email_address not in identities:
        ...
        email = get_restmail(
            ...
            and "Email Address Verification Request" in x["headers"]["subject"]
            and EMAIL_VERIFICATION_PATTERN.match(email["text"]) is not None,
        )
        ...
        return (
            ...
            and "://aws.amazon.com/ses/verifysuccess" in result.headers.get("location")
        )
    else:
        return True
...
def test_all():
    ...
    * TODO Check for attachment
    ...
    result = requests.request("delete", f"https://restmail.net/mail/{RESTMAIL_USER}")
    ...
    email = get_restmail(RESTMAIL_USER, lambda x: subject in x["headers"]["subject"])
    ...
    reply_quote = "".join(
        str(x)
        for x in (soup.body.contents if soup.body is not None else soup.contents)
...
<div class="gmail_quote">
    ...
    fields = {
        "date": (
            reply_datetime.strftime("%a, {} %b %Y").format(
                reply_datetime.strftime("%d").strip("0")
            )
        ),
    ...
    result = client.send_email(
        ...
        Message={
            ...
            "Body": {"Html": {"Data": reply.format(**fields)}},
        },
    )
    ...
    while checks <= 12:
        checks += 1
        status, comments = (
            gh.repos[REPO_OWNER][REPO_NAME].issues[issue_number].comments.get()
        )
